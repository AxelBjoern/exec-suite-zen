...
// ─────────────────────────────────────────────────────────────────────────
// Multi-turn agent tool loop (callAgentTool)
//
// Strategy by model:
//   • Native tool models (Grok, GPT, Claude, DeepSeek): use OpenRouter
//     `tools` field with an assistant↔tool message loop, max N turns.
//   • Hermes 4 405B: no tool-capable endpoint on OpenRouter, so we use a
//     JSON envelope: the model must reply with
//       { "tool": "<name>", "args": { ... } }
//     or
...
function isHermes(model: string) { return model === HERMES_SLUG; }
...
async function callAgentToolNative(args: {
  ...
  for (let turn = 0; turn < args.maxTurns; turn++) {
    ...
    // assistant message must be pushed (with tool_calls) before tool replies
    messages.push({
      role: "assistant",
      content: msg.content ?? "",
    ...
    for (const tc of toolCalls) {
      ...
      parsedArgs = p.ok ? p.value : { __parse_error: (p as any).error?.message ?? "invalid json" };
      const exec = await executeToolCall(name, parsedArgs, args.ctx);
      const result = exec.ok ? exec.result : { error: exec.error };
      invocations.push({ name, args: parsedArgs, result: exec.ok ? exec.result : undefined, error: exec.ok ? undefined : exec.error });
      messages.push({
        role: "tool",
        tool_call_id: tc.id,
        name,
...
async function callAgentToolHermes(args: {
  ...
  for (let turn = 0; turn < args.maxTurns; turn++) {
    ...
    const result = exec.ok ? exec.result : { error: exec.error };
    invocations.push({ name: env.tool, args: env.args, result: exec.ok ? exec.result : undefined, error: exec.ok ? undefined : exec.error });
